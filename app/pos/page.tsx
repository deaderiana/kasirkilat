...
export default function CashierPage() {
  ...
  // --- CART LOGIC (FIX: STOCK GUARD ADDED) ---
  ...
  const processTransaction = async (phone: string, customerName: string, customerId: number | null = null) => {
    ...
    message += `Kasir: ${cashierDisplayName}\n`;
    message += `Plg: ${customerName}\n\n`;
  ...
  const handleOpenShift = async (e: React.FormEvent) => {
      ...
      const name = cashierNameInput.trim() || 'Admin'; 
      ...
      alert(`Shift Dibuka. Selamat bertugas, ${name}!`);
  };
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gray-50 font-sans font-inter text-gray-800 overflow-hidden relative">
      ...
      {/* --- MENU & MODALS (UPDATED: HISTORY MODAL WITH SCROLL) --- */}
      {showMainMenu && (
          <div className="fixed inset-0 bg-black/60 z-[100] flex justify-start">
              <div className="bg-white w-72 h-full shadow-2xl p-6 flex flex-col animate-in slide-in-from-left duration-300">
                  ...
                  <div className="border-t border-gray-100 pt-4"><button onClick={handleLogout} className="w-full flex items-center gap-3 p-3 rounded-xl text-gray-500 font-bold hover:bg-gray-50 transition"><LogOut size={20}/> Keluar Aplikasi</button><p className="text-[10px] text-center text-gray-300 mt-4">Versi 1.9.5 (StockGuard & MobileFix)</p></div>
              </div>
      ...
      {/* HISTORY MODAL (FIXED WITH HORIZONTAL SCROLL) */}
      {showHistoryModal && (
          <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4 backdrop-blur-sm">
              <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
                  <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
                      <h3 className="font-bold text-gray-800 flex items-center gap-2"><History size={20}/> Riwayat Hari Ini</h3>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      {recentTrx.length === 0 ? <p className="text-center text-gray-400 text-sm">Belum ada transaksi.</p> : recentTrx.map(trx => (
                          // Added overflow-x-auto to the container to allow scrolling
                          <div key={trx.id} className="overflow-x-auto">
                              <div className="p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition flex justify-between items-center min-w-[300px]">
                                  <div>
                                      ...
                                      <p className="text-[10px] text-gray-400 truncate max-w-[150px]">{trx.items_summary}</p>
                                  </div>
                                  <div className="flex gap-2">
