...
export default function AdminPage() {
  ...
  const [locations, setLocations] = useState<any[]>([]); // NEW: Master Lokasi
  ...
  const [showLocationModal, setShowLocationModal] = useState(false); // NEW: Modal Lokasi
  const [newLocationName, setNewLocationName] = useState(""); // NEW: Form Lokasi
  ...
  const fetchAllData = async () => {
    ...
    if (locData && locData.length > 0) { 
        ...
        // Set default location to first one if empty
        if (productForm.location === '') setProductForm(prev => ({ ...prev, location: locData[0].name })); 
    } else {
        setLocations([]); // User must add location
    }
  ...
  // --- UPDATED PRODUCT SUBMIT (USING DROPDOWN LOCATION) ---
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    // Jika lokasi belum dipilih, pakai yang pertama atau string kosong
    ...
    if (!error) { 
        ...
        // Reset location ke item pertama
  ...
  // --- PRINT LOGIC (THERMAL 58MM FIXED) ---
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans font-inter text-gray-800">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8">
            ...
            {/* WMS & CUSTOMER TABS (SAME AS BEFORE) */}
            {activeTab === 'customers' && (
            ...
            {/* --- PRODUCTS TAB (UPDATED LOCATION DROPDOWN) --- */}
