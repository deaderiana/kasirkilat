...
import { 
  ...
  Users, ShoppingBag, ClipboardList, CheckCircle, Shield, Key
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [adminPin, setAdminPin] = useState<string>(''); // NEW: PIN State
  ...
  useEffect(() => {
    const checkUser = async () => {
      // Cek Session Storage untuk keamanan
      ...
      if (!session) { router.push('/login'); } 
        ...
        if (profile) {
            ...
            // SECURITY CHECK: Jika PIN ada, tapi belum unlock di session, tendang ke kasir
            if (profile.admin_pin && isUnlocked !== 'true') {
                router.push('/'); 
                return;
            }
        }
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
      ...
      if (adminPin && !/^\d+$/.test(adminPin)) {
          alert("PIN harus berupa angka!"); setLoading(false); return;
      }
      ...
      if (error) alert("Gagal simpan: " + error.message); 
          ...
          // Auto unlock session setelah save PIN
  ...
  const handleAddSupplier = async (e: React.FormEvent) => {
      e.preventDefault(); setLoading(true);
      const { error } = await supabase.from('suppliers').insert([{ ...supplierForm, user_id: user.id }]);
      if(!error) { alert("Supplier ditambahkan!"); setSupplierForm({name:'', phone:'', address:''}); fetchAllData(); } else alert(error.message); setLoading(false);
  };
  ...
  const handleRestock = async (e: React.FormEvent) => {
      e.preventDefault(); if (!isPro) { handleUpgradeRequest('new'); return; } setLoading(true);
      const cost = Number(purchaseForm.cost_price); const qty = Number(purchaseForm.qty); const total = cost * qty;
      try {
          ...
          if(prod) {
              ...
              await supabase.from('stock_logs').insert([{ product_id: purchaseForm.product_id, type: 'IN', qty: qty, note: `Beli dari Supplier (ID: ${pur.id})` }]);
          }
          alert("Stok Berhasil Ditambah!"); setPurchaseForm({ ...purchaseForm, qty: '', cost_price: '' }); fetchAllData();
      } catch (err: any) { alert("Gagal: " + err.message); } setLoading(false);
  };
  const handleOpname = async (e: React.FormEvent) => {
      e.preventDefault(); if (!isPro) { handleUpgradeRequest('new'); return; } setLoading(true);
      const realStock = Number(opnameForm.actual_stock); const prodId = opnameForm.product_id;
      try {
          ...
          if (prod) {
              ...
              await supabase.from('stock_logs').insert([{ product_id: prodId, type: diff >= 0 ? 'IN' : 'OUT', qty: Math.abs(diff), note: `Stock Opname: ${opnameForm.reason}` }]);
              alert("Stok Fisik Diupdate!"); setOpnameForm({ ...opnameForm, actual_stock: '', reason: '' }); fetchAllData();
          }
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { ...productForm, category: cat, price: Number(productForm.price), buy_price: Number(productForm.buy_price) || 0, stock: Number(productForm.stock) || 0 };
    let error;
    if (editingProduct) { 
        const diff = payload.stock - editingProduct.stock;
        const { error: e } = await supabase.from('products').update(payload).eq('id', editingProduct.id); error = e;
        ...
        const { data, error: e } = await supabase.from('products').insert([payload]).select().single(); error = e;
    ...
    if (!error) { alert('Berhasil!'); setEditingProduct(null); setProductForm({ name: '', price: '', buy_price: '', stock: '', category: categories[0]?.name || '', image: '' }); fetchAllData(); } else { alert(error.message); } setLoading(false);
  };
  ...
  const handleDeleteTransaction = async (id: number) => {
    if (!confirm("Yakin hapus transaksi ini? Stok akan dikembalikan otomatis.")) return; setLoading(true);
    try {
        ...
        await supabase.from('transactions').delete().eq('id', id); alert("Dihapus & Stok Kembali!"); fetchAllData();
  ...
  const handleDownloadExcel = async () => { 
      if (!isPro) return handleUpgradeRequest('new'); const workbook = new ExcelJS.Workbook(); 
      const wsTrx = workbook.addWorksheet('Penjualan'); wsTrx.columns = [{ header: 'Tanggal', key: 'd' }, { header: 'Detail', key: 'k' }, { header: 'Pelanggan', key: 'p' }, { header: 'Total', key: 't' }, { header: 'Pajak', key: 'tax' }]; transactions.forEach(t => wsTrx.addRow({ d: new Date(t.created_at).toLocaleDateString(), k: t.items_summary, p: t.customer_name || '-', t: t.total_amount, tax: t.tax || 0 })); 
      const wsExp = workbook.addWorksheet('Pengeluaran'); wsExp.columns = [{ header: 'Tanggal', key: 'd' }, { header: 'Keperluan', key: 'k' }, { header: 'Kategori', key: 'c' }, { header: 'Jumlah', key: 'a' }]; expenses.forEach(e => wsExp.addRow({ d: new Date(e.created_at).toLocaleDateString(), k: e.title, c: e.category, a: e.amount })); 
      const wsPur = workbook.addWorksheet('Pembelian'); wsPur.columns = [{ header: 'Tanggal', key: 'd' }, { header: 'Supplier', key: 's' }, { header: 'Total Biaya', key: 't' }]; purchases.forEach(p => wsPur.addRow({ d: new Date(p.created_at).toLocaleDateString(), s: p.suppliers?.name, t: p.total_cost }));
      const wsSup = workbook.addWorksheet('Data Supplier'); wsSup.columns = [{ header: 'Nama', key: 'n' }, { header: 'Telepon', key: 'p' }, { header: 'Alamat', key: 'a' }]; suppliers.forEach(s => wsSup.addRow({ n: s.name, p: s.phone, a: s.address }));
      const wsLog = workbook.addWorksheet('Laporan Arus Stok'); wsLog.columns = [{ header: 'Waktu', key: 'd' }, { header: 'Produk', key: 'p' }, { header: 'Tipe', key: 't' }, { header: 'Jumlah', key: 'q' }, { header: 'Keterangan', key: 'n' }]; allStockLogs.forEach(l => { wsLog.addRow({ d: new Date(l.created_at).toLocaleDateString(), p: l.products?.name, t: l.type, q: l.qty, n: l.note }); });
      const buffer = await workbook.xlsx.writeBuffer(); const url = window.URL.createObjectURL(new Blob([buffer])); const a = document.createElement('a'); a.href = url; a.download = `Laporan_Lengkap.xlsx`; a.click(); 
  };
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans font-inter text-gray-800">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col h-screen`}>
        ...
        <div className="p-4 space-y-1 flex-1 overflow-y-auto scrollbar-hide">
            ...
            {/* BUTTON LOCK APP (NEW) */}
            <button onClick={handleLockApp} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-red-500 hover:bg-red-50 hover:text-red-700 transition"><Shield size={20} /> Lock (Mode Kasir)</button>
        </div>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8">
            {/* EXISTING TABS (Dashboard, Products, etc.) - Sama seperti sebelumnya */}
                    ...
                    <form onSubmit={handleSaveSettings} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 relative">
                                <Key className="absolute left-3 text-gray-400" size={18}/>
                                <input 
                                    type="password" 
                                    value={adminPin} 
                                    onChange={e => setAdminPin(e.target.value)}
                                    className="flex-1 pl-10 p-3 bg-gray-50 rounded-xl border border-gray-200 font-bold"
                                    placeholder="Atur 6 digit PIN"
                                    maxLength={6}
                                />
                            </div>
