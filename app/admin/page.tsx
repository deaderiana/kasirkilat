...
import { 
  ...
  Users, ShoppingBag, ClipboardList, CheckCircle, Shield, Key, Zap
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleProductSubmit = async (e: React.FormEvent) => { e.preventDefault(); setLoading(true); if (!isPro && !editingProduct && products.length >= 1000) { if(confirm("LIMIT TERCAPAI! Upgrade PRO?")) handleUpgradeRequest('new'); setLoading(false); return; } const cat = productForm.category || (categories.length > 0 ? categories[0].name : 'Uncategorized'); const payload = { ...productForm, category: cat, price: Number(productForm.price), buy_price: Number(productForm.buy_price) || 0, stock: Number(productForm.stock) || 0 }; let error; if (editingProduct) { const diff = payload.stock - editingProduct.stock; const { error: e } = await supabase.from('products').update(payload).eq('id', editingProduct.id); error = e; if (!e && diff !== 0) await supabase.from('stock_logs').insert([{ product_id: editingProduct.id, type: diff > 0 ? 'IN' : 'OUT', qty: Math.abs(diff), note: diff > 0 ? 'Restock Admin' : 'Koreksi Stok' }]); } else { const { data, error: e } = await supabase.from('products').insert([payload]).select().single(); error = e; if (!e && data && payload.stock > 0) await supabase.from('stock_logs').insert([{ product_id: data.id, type: 'IN', qty: payload.stock, note: 'Stok Awal' }]); } if (!error) { alert('Berhasil!'); setEditingProduct(null); setProductForm({ name: '', price: '', buy_price: '', stock: '', category: categories[0]?.name || '', image: '' }); fetchAllData(); } else { alert(error.message); } setLoading(false); };
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans font-inter text-gray-800">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col h-screen`}>
        ...
        {/* FOOTER FIXED - Upgrade Button Here */}
        <div className="p-4 border-t border-gray-100 flex-shrink-0 space-y-2">
            {!isPro && (
                <div onClick={()=>handleUpgradeRequest('new')} className="w-full bg-gradient-to-r from-amber-400 to-orange-500 text-white p-3 rounded-xl cursor-pointer shadow-lg hover:scale-105 transition flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <Crown size={20} className="text-white"/>
                        <div className="text-left">
                            <p className="text-xs font-bold">UPGRADE PRO</p>
                            <p className="text-[9px] opacity-90">Buka semua fitur!</p>
                        </div>
                    </div>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-8">
            {/* ... SEMUA TAB (Dashboard s/d Settings) SAMA SEPERTI SEBELUMNYA ... */}
            {/* Karena panjang, saya singkat bagian konten tabs yang tidak berubah */}
            {/* Copy paste konten tabs dari script sebelumnya di sini */}
